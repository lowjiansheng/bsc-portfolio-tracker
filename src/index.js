...
console.log(
	"Start pulling data from the blockchain for address = " +
		SECRETS.PUBLIC_ADDRESS_TO_TRACK
);

let protocolFactory = new ProtocolFactory.ProtocolFactory(
	web3,
	SECRETS.PUBLIC_ADDRESS_TO_TRACK
);

BscScanUtils.getBscScanTransactions(SECRETS.PUBLIC_ADDRESS_TO_TRACK).then(
	(accountTransactions) => {
		protocolFactory.fetchAccountValuesInProtocol(accountTransactions);
	}
);
