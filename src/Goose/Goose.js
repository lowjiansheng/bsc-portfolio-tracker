...
function Goose (web3) {
        ...
        return this.getLPsParticipated(userAddress).then((participatedLPs) => {
            ...
            participatedLPs.forEach((participatedLP) => {
                participatedLPsPricePromise.push(
                    LPTokenCalculator.isLPToken(this.web3, participatedLP.lpToken).then((isLP) => {
                        if (isLP) {
                            return LPTokenCalculator.getPriceOfLPToken(this.web3, participatedLP.lpToken).then((pricePerLPToken) => {
                                return TokenInfoFetcher.getTokenInfoFromAddress(this.web3, participatedLP.lpToken).then((tokenInfo) => {
                                    return (participatedLP.amount / (10 ** tokenInfo.decimals)) * pricePerLPToken
                                }) 
                            })
                        } else {
                            return PriceFetcher.getPriceByTokenAddress(this.web3, participatedLP.lpToken).then((pricePerToken) => {
                                return TokenInfoFetcher.getTokenInfoFromAddress(this.web3, participatedLP.lpToken).then((tokenInfo) => {
    ...
    this.getLPsParticipated = function(userAddress) {
        let contract = new this.web3.eth.Contract(IGooseChef, GooseConstants.GOOSE_CHEF_CONTRACT)
