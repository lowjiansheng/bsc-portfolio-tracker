...
var getPairInformationWithPairAddress = function(web3, pairAddress) {
    ...
    return contract.methods.getReserves().call().then((reserves) => {
        return contract.methods.token0().call().then((token0Address) => {
            return contract.methods.token1().call().then((token1Address) => {
                return contract.methods.totalSupply().call().then((totalSupply) => {
...
var isTokenAnLPToken = function(web3, address) { 
    ...
    return contract.methods.token0().call().then(() => {
        return true
    })
    .catch(() => {
        return false
...
var getPairContractWithTokensAddress = function(web3, factoryAddress, tokenA, tokenB) {
...
var getAmountDepositedIntoPair = function(web3, userAddress, pairAddress) {
    getPairInformationWithPairAddress(web3, pairAddress).then((pairInformation) => {
        let ercTokenAddress 
        if (pairInformation.token0Address.toLowerCase() === Constants.WBNB_ADDRESS.toLowerCase()) {
            ercTokenAddress = pairInformation.token1Address
        } else if (pairInformation.token1Address.toLowerCase() === Constants.WBNB_ADDRESS.toLowerCase()) {
...
var getBEP20TransactionsByAddress = function(userAddress) {
    return new Promise((resolve, reject) => {
        let options = {
            ...
            path: "/api?module=account&action=tokentx&address=" + userAddress + "&startblock=0&endblock=999999999&sort=asc&apikey=YourApiKeyToken"
        }
